...
중간가격은 Brownian motion을 따른다.
dS = sigma * dW

이산화하면 다음과 같다.
S(t+dt) = S(t) + sigma * sqrt(dt) * Z
여기서 Z는 표준정규분포를 따르는 난수다.

구현에서는 np.random.randn을 사용하여 난수를 생성하고, sigma * sqrt(dt)를 곱한 후 누적합으로 경로를 생성한다.
...
avellaneda_stoikov_simulation.ipynb 파일에서 다음 부분을 참조한다.

SimulationConfig 클래스에서 mode를 paper로 설정하면 Paper 모드가 활성화된다.
paper_n_steps, sigma, gamma, A, k 파라미터가 Paper 모드에서 사용된다.

MarketDataManager 클래스의 _setup_paper_mode_paths 메서드에서 GBM 경로를 생성한다.

QuoteEngine 클래스의 compute_reservation_price 메서드에서 mode가 paper일 때 논문 원본 수식을 적용한다.
compute_optimal_spread_paper 메서드에서 논문 식 (30)을 계산한다.
compute_fill_probability_paper 메서드에서 논문 식 (12)를 계산한다.
