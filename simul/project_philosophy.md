...
1분 스냅샷 방식을 채택한다. 1시간에 60행으로 데이터 관리가 용이하다. 호가, 그릭스 등 시장 상태는 스냅샷 시점 값을 저장한다. 체결량은 1분간 누적 체결량을 계산하여 저장한다.
...
1분체결량은 현재 누적거래량에서 이전 누적거래량을 빼서 계산하며, 거래 강도를 파악하는 용도이다. 체결방향은 체결가와 호가를 비교하여 매수/매도 주도를 판단한다. 매수주도체결량은 체결방향이 BUY일 때 1분체결량으로 매수 압력을 측정한다. 매도주도체결량은 체결방향이 SELL일 때 1분체결량으로 매도 압력을 측정한다.
